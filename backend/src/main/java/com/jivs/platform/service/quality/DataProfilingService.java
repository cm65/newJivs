...
public class DataProfilingService {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(DataProfilingService.class);
    ...
    public DataProfile profile(Map<String, Object> data) {
        ...
        profile.setTotalRecords((long) calculateRecordCount(data));
        profile.setTotalFields((long) data.size());
        ...
        // Note: fieldProfiles, completeness, uniqueness, patternCompliance, patterns, and distributions
        // are not stored in the DataProfile entity but are calculated on-the-fly

        // Calculate overall statistics (for logging)
        double completeness = calculateCompleteness(fieldProfiles);
        double uniqueness = calculateUniqueness(fieldProfiles);
        double patternCompliance = calculatePatternCompliance(fieldProfiles);

        log.debug("Profile stats - Completeness: {}, Uniqueness: {}, Pattern Compliance: {}",
                  completeness, uniqueness, patternCompliance);

        // Identify data patterns (for logging)
        List<DataPattern> patterns = identifyPatterns(data);
        log.debug("Identified {} data patterns", patterns.size());

        // Detect data distributions (for logging)
        Map<String, Distribution> distributions = analyzeDistributions(fieldProfiles);
        log.debug("Analyzed {} field distributions", distributions.size());
